...
def kernel32_ContinueDebugEvent(jitter):
    ...
    BOOL ContinueDebugEvent(
        DWORD dwProcessId,
        DWORD dwThreadId,
        [ContinueStatusFlag] dwContinueStatus
    )
    """
...
def kernel32_GetEnabledExtendedFeatures(jitter):
    """
    DWORD64 GetEnabledExtendedFeatures(
        DWORD64 FeatureMask
    )
    """
...
def kernel32_GetExtendedFeaturesMask(jitter):
    """
    DWORD64 GetExtendedFeaturesMask(
...
def kernel32_InitializeExtendedContext(jitter):
    """
    BOOL InitializeExtendedContext(
        PVOID Context,
        DWORD ContextFlags,
        PCONTEXT_EX* ContextEx
    )
...
def kernel32_SetDefaultDllDirectories(jitter):
    ...
    BOOL SetDefaultDllDirectories(
        [LOAD_LIBRARY_SEARCH_FLAGS] DirectoryFlags
    )
    """
...
def kernel32_CreateToolhelp32Snapshot(jitter):
    """
    [FILE_HANDLE] CreateToolhelp32Snapshot(
...
def kernel32_RegisterApplicationRecoveryCallback(jitter):
    ...
    HRESULT RegisterApplicationRecoveryCallback(
        ...
        PVOID pvParameter,
        DWORD dwPingInterval,
        [ApplicationRestartFlags] dwFlags
    )
    """
...
def kernel32_CreateFileTransacted(jitter, get_str, set_str):
    """
    [FILE_HANDLE] CreateFileTransacted(
        LPCTSTR lpFileName,
        [FILE_ACCESS_MASK] dwDesiredAccess,
        [FILE_SHARE_MODE] dwShareMode,
        LPSECURITY_ATTRIBUTES lpSecurityAttributes,
...
def kernel32_FindFirstFileNameTransactedW(jitter):
    """
    [FILE_HANDLE] FindFirstFileNameTransactedW(
        LPCWSTR lpFileName,
        DWORD dwFlags,
...
def kernel32_FindFirstFileNameW(jitter):
    """
    [FILE_HANDLE] FindFirstFileNameW(
        LPCWSTR lpFileName,
        DWORD dwFlags,
        LPDWORD StringLength,
        PWCHAR LinkName
...
def kernel32_FindFirstFileTransacted(jitter, get_str, set_str):
    """
    [FILE_HANDLE] FindFirstFileTransacted(
        LPCTSTR lpFileName,
        FINDEX_INFO_LEVELS fInfoLevelId,
        LPVOID lpFindFileData,
        FINDEX_SEARCH_OPS fSearchOp,
...
def kernel32_FindFirstStreamTransactedW(jitter):
    """
    [FILE_HANDLE] FindFirstStreamTransactedW(
        LPCWSTR lpFileName,
...
def kernel32_GetBinaryType(jitter, get_str, set_str):
    ...
    BOOL GetBinaryType(
        ...
        [GetBinaryTypeResult*] lpBinaryType
    )
    """
...
def kernel32_OpenFileById(jitter):
    """
    [FILE_HANDLE] OpenFileById(
        HANDLE hFile,
        LPFILE_ID_DESCRIPTOR lpFileID,
        [FILE_ACCESS_MASK] dwDesiredAccess,
        [FILE_SHARE_MODE] dwShareMode,
        LPSECURITY_ATTRIBUTES lpSecurityAttributes,
        [FlagsAndAttributes] dwFlags
    )
    """
...
def kernel32_MapViewOfFile(jitter):
    ...
    LPVOID MapViewOfFile(
        ...
        [MapAccessFlags] dwDesiredAccess,
        DWORD dwFileOffsetHigh,
        DWORD dwFileOffsetLow,
...
def kernel32_NeedCurrentDirectoryForExePath(jitter, get_str, set_str):
    """
    BOOL NeedCurrentDirectoryForExePath(
        LPCTSTR ExeName
    )
    """
...
def kernel32_OpenProcess(jitter):
    """
    HANDLE OpenProcess(
        [PROCESS_ACCESS_MASK] dwDesiredAccess,
        BOOL bInheritHandle,
        DWORD dwProcessId
    )
    """
...
def kernel32_OpenThread(jitter):
    """
    HANDLE OpenThread(
        [THREAD_ACCESS_MASK] dwDesiredAccess,
        BOOL bInheritHandle,
        DWORD dwThreadId
    )
    """
...
def kernel32_FindActCtxSectionGuid(jitter):
    ...
    BOOL FindActCtxSectionGuid(
        ...
        [ACTIVATION_CONTEXT_SECTION] ulSectionId,
...
def kernel32_QueryActCtxW(jitter):
    """
    BOOL QueryActCtxW(
        [QueryActCtxFlags] dwFlags,
        HANDLE hActCtx,
        PVOID pvSubInstance,
        [ActivationContextInfoClass] ulInfoClass,
        PVOID pvBuffer,
        SIZE_T cbBuffer,
...
def kernel32_GetCommModemStatus(jitter):
    ...
    BOOL GetCommModemStatus(
        HANDLE hFile,
        [MODEM_STATUS_FLAGS*] lpModemStat
    )
    """
...
def kernel32_CreateConsoleScreenBuffer(jitter):
    ...
    HANDLE CreateConsoleScreenBuffer(
        ...
        [CONSOLE_TEXTMODE_FLAGS] dwFlags,
...
def kernel32_GetConsoleAliasExes(jitter, get_str, set_str):
    """
    DWORD GetConsoleAliasExes(
        LPTSTR lpExeNameBuffer,
...
def kernel32_GetConsoleDisplayMode(jitter):
    ...
    BOOL GetConsoleDisplayMode(
        [CONSOLE_FULLSCREEN_MODE*] lpModeFlags
    )
    """
...
def kernel32_GetThreadErrorMode(jitter):
    """
    [SEM_FLAGS_DWORD] GetThreadErrorMode()
    """
    ret_ad, args = jitter.func_args_stdcall([])
    raise RuntimeError('API not implemented')
    jitter.func_ret_stdcall(ret_ad, ret_value)
...
def kernel32_SetThreadErrorMode(jitter):
    ...
    BOOL SetThreadErrorMode(
        [SEM_FLAGS_DWORD] dwNewMode,
        [SEM_FLAGS_DWORD*] lpOldMode
    )
    """
...
def kernel32_DuplicateHandle(jitter):
    ...
    BOOL DuplicateHandle(
        ...
        [PROCESS_THREAD_HANDLE] hSourceHandle,
        [ProcessHandle] hTargetProcessHandle,
        LPHANDLE lpTargetHandle,
        [StandardAccessRights] dwDesiredAccess,
        BOOL bInheritHandle,
        [DUPLICATE_HANDLE_FLAGS] dwOptions
    )
    """
...
def kernel32_CompareString(jitter, get_str, set_str):
    ...
    int CompareString(
        LCID Locale,
        [NLS_STRING_COMPARE_FLAGS] dwCmpFlags,
        LPCTSTR lpString1,
        int cchCount1,
        LPCTSTR lpString2,
        int cchCount2
...
def kernel32_EnumCalendarInfo(jitter, get_str, set_str):
    ...
    BOOL EnumCalendarInfo(
        CALINFO_ENUMPROC pCalInfoEnumProc,
        LCID Locale,
        CALID Calendar,
        CALTYPE CalType
    )
...
def kernel32_EnumCalendarInfoEx(jitter, get_str, set_str):
    ...
    BOOL EnumCalendarInfoEx(
        CALINFO_ENUMPROCEX pCalInfoEnumProcEx,
        LCID Locale,
        CALID Calendar,
        CALTYPE CalType
    )
...
def kernel32_EnumDateFormatsEx(jitter, get_str, set_str):
    ...
    BOOL EnumDateFormatsEx(
        DATEFMT_ENUMPROCEX lpDateFmtEnumProcEx,
        LCID Locale,
        [NLS_DATE_FLAGS] dwFlags
    )
    """
...
def kernel32_EnumSystemCodePages(jitter, get_str, set_str):
    ...
    BOOL EnumSystemCodePages(
        ...
        [CodePageEnumerationFlags] dwFlags
    )
    """
...
def kernel32_EnumSystemLocales(jitter, get_str, set_str):
    ...
    BOOL EnumSystemLocales(
        ...
        [LocaleEnumerationFlags] dwFlags
    )
    """
...
def kernel32_EnumSystemLocalesEx(jitter):
    ...
    BOOL EnumSystemLocalesEx(
        ...
        [EnumSystemLocalesExFlags] dwFlags,
        LPARAM lParam,
        LPVOID lpReserved
    )
    """
...
def kernel32_FindNLSString(jitter):
    ...
    int FindNLSString(
        LCID Locale,
        [NLS_STRING_FIND_FLAGS] dwFindNLSStringFlags,
...
def kernel32_FindStringOrdinal(jitter):
    ...
    int FindStringOrdinal(
        [NLS_FIND_FLAGS] dwFindStringOrdinalFlags,
        LPCWSTR lpStringSource,
        int cchSource,
        LPCWSTR lpStringValue,
        int cchValue,
...
def kernel32_GetCalendarSupportedDateRange(jitter):
    ...
    BOOL GetCalendarSupportedDateRange(
        CALID Calendar,
        LPCALDATETIME lpCalMinDateTime,
        LPCALDATETIME lpCalMaxDateTime
    )
    """
...
def kernel32_GetCurrencyFormat(jitter, get_str, set_str):
    ...
    int GetCurrencyFormat(
        LCID Locale,
        [LOCALE_NOUSEROVERRIDE_FLAG] dwFlags,
        LPCTSTR lpValue,
...
def kernel32_GetStringScripts(jitter):
    ...
    int GetStringScripts(
        [GET_STRING_SCRIPTS_FLAGS] dwFlags,
        LPCWSTR lpString,
        int cchString,
        LPWSTR lpScripts,
        int cchScripts
...
def kernel32_LoadStringByReference(jitter):
    ...
    BOOL LoadStringByReference(
        [LoadStringByReference_Flags] Flags,
        PCWSTR Language,
        PCWSTR SourceString,
        PWSTR Buffer,
        ULONG cchBuffer,
...
def kernel32_SleepConditionVariableSRW(jitter):
    ...
    BOOL SleepConditionVariableSRW(
        ...
        PSRWLOCK SRWLock,
        [WaitTimeout] dwMilliseconds,
        [CONDITION_VARIABLE_FLAGS] Flags
    )
    """
...
def kernel32_CreateEventEx(jitter, get_str, set_str):
    ...
    HANDLE CreateEventEx(
        LPSECURITY_ATTRIBUTES lpEventAttributes,
        LPCTSTR lpName,
        [CREATE_EVENT_FLAGS] dwFlags,
        [EventAccessRights] dwDesiredAccess
    )
    """
...
def kernel32_CreateMutexEx(jitter, get_str, set_str):
    ...
    HANDLE CreateMutexEx(
        LPSECURITY_ATTRIBUTES lpMutexAttributes,
        LPCTSTR lpName,
        [CREATE_MUTEX_FLAGS] dwFlags,
        [MutexAccessRights] dwDesiredAccess
    )
    """
...
def kernel32_CreateSemaphoreEx(jitter, get_str, set_str):
    ...
    HANDLE CreateSemaphoreEx(
        ...
        LPCTSTR lpName,
        DWORD dwFlags,
        [SemaphoreAccessRights] dwDesiredAccess
    )
    """
...
def kernel32_InterlockedPushListSList(jitter):
    """
    PSLIST_ENTRY InterlockedPushListSList(
        PSLIST_HEADER ListHead,
        PSLIST_ENTRY List,
...
def kernel32_EnterSynchronizationBarrier(jitter):
    ...
    BOOL EnterSynchronizationBarrier(
        LPSYNCHRONIZATION_BARRIER lpBarrier,
        [SYNCHRONIZATION_BARRIER_FLAGS] dwFlags
    )
    """
...
def kernel32_WaitForMultipleObjectsEx(jitter):
    """
    [WAIT_RESULT] WaitForMultipleObjectsEx(
        DWORD nCount,
        const HANDLE* lpHandles,
        BOOL bWaitAll,
        [WaitTimeout] dwMilliseconds,
...
def kernel32_CreateWaitableTimerEx(jitter, get_str, set_str):
    ...
    HANDLE CreateWaitableTimerEx(
        LPSECURITY_ATTRIBUTES lpTimerAttributes,
        LPCTSTR lpTimerName,
        [CREATE_WAITABLE_TIMER_FLAGS] dwFlags,
        [TimerAccessRights] dwDesiredAccess
    )
    """
...
def kernel32_GetFirmwareEnvironmentVariableEx(jitter, get_str, set_str):
    """
    DWORD GetFirmwareEnvironmentVariableEx(
        LPCTSTR lpName,
        LPCTSTR lpGuid,
        PVOID pBuffer,
        DWORD nSize,
...
def kernel32_GetProductInfo(jitter):
    ...
    BOOL GetProductInfo(
        ...
        [ProductType*] pdwReturnedProductType
    )
    """
...
def kernel32_IsProcessorFeaturePresent(jitter):
    ...
    BOOL IsProcessorFeaturePresent(
        [ProcessorFeatureEnum] ProcessorFeature
    )
    """
...
def kernel32_VerSetConditionMask(jitter):
    ...
    ULONGLONG VerSetConditionMask(
        ...
        [NtVerType] dwTypeBitMask,
        [VerOperator] dwConditionMask
    )
    """
...
def kernel32_CreateTapePartition(jitter):
    """
    [ERROR_CODE] CreateTapePartition(
        HANDLE hDevice,
...
def kernel32_RaiseFailFastException(jitter):
    ...
    VOID RaiseFailFastException(
        ...
        [FAIL_FAST_EXCEPTION_FLAGS] dwFlags
    )
    """
...
def kernel32_WerSetFlags(jitter):
    """
    HRESULT WerSetFlags(
        [WER_FAULT_REPORTING_FLAGS] dwFlags
    )
    """
    ret_ad, args = jitter.func_args_stdcall(["dwFlags"])
    raise RuntimeError('API not implemented')
...
def kernel32_FindFirstChangeNotification(jitter, get_str, set_str):
    ...
    HANDLE FindFirstChangeNotification(
        LPCTSTR lpPathName,
        BOOL bWatchSubtree,
        [FILE_NOTIFY_CHANGE_FLAGS] dwNotifyFilter
    )
    """
...
def kernel32_GetVolumeInformation(jitter, get_str, set_str):
    ...
    BOOL GetVolumeInformation(
        ...
        [LPFILE_SYSTEM_FLAGS] lpFileSystemFlags,
...
def kernel32_DisableThreadProfiling(jitter):
    """
    [ERROR_CODE] DisableThreadProfiling(
...
def kernel32_EnableThreadProfiling(jitter):
    """
    [ERROR_CODE] EnableThreadProfiling(
        [ThreadHandle] ThreadHandle,
        [THREAD_PROFILING_FLAGS] Flags,
...
def kernel32_QueryThreadProfiling(jitter):
    """
    [ERROR_CODE] QueryThreadProfiling(
        [ThreadHandle] ThreadHandle,
        PBOOLEAN Enabled
    )
    """
...
def kernel32_ReadThreadProfilingData(jitter):
    """
    [ERROR_CODE] ReadThreadProfilingData(
        HANDLE PerformanceDataHandle,
        [READ_THREAD_PROFILING_FLAGS] Flags,
...
def kernel32_BaseFormatTimeOut(jitter):
    """
    LARGE_INTEGER* BaseFormatTimeOut(
...
def kernel32_BaseGetNamedObjectDirectory(jitter):
    """
    NTSTATUS BaseGetNamedObjectDirectory(
        HANDLE* phDir
    )
    """
...
def kernel32_Basep8BitStringToDynamicUnicodeString(jitter):
    """
    BOOL Basep8BitStringToDynamicUnicodeString(
...
def kernel32_CheckElevationEnabled(jitter):
    """
    [ERROR_CODE] CheckElevationEnabled(
        BOOL* pResult
    )
    """
...
def kernel32_SetConsoleMaximumWindowSize(jitter):
    """
    BOOL SetConsoleMaximumWindowSize(
        HANDLE hConsole,
        DWORD dwUnk
    )
    """
...
def kernel32_AddLocalAlternateComputerName(jitter):
    """
    [ERROR_CODE] AddLocalAlternateComputerName(
        LPCTSTR lpDnsFQHostname,
        ULONG ulFlags
    )
    """
...
def kernel32_ResolveDelayLoadedAPI(jitter):
    ...
    PVOID ResolveDelayLoadedAPI(
        [HMODULE-PVOID] ParentModuleBase,
...
def kernel32_ResolveDelayLoadsFromDll(jitter):
    """
    NTSTATUS ResolveDelayLoadsFromDll(
        [HMODULE-PVOID] ParentBase,
...
def kernel32_ClosePackageInfo(jitter):
    """
    [ERROR_CODE_LONG] ClosePackageInfo(
...
def kernel32_GetApplicationUserModelId(jitter):
    """
    [ERROR_CODE_LONG] GetApplicationUserModelId(
        HANDLE hProcess,
...
def kernel32_GetCurrentApplicationUserModelId(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentApplicationUserModelId(
        UINT32* applicationUserModelIdLength,
        PWSTR applicationUserModelId
    )
    """
...
def kernel32_GetCurrentPackageFamilyName(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentPackageFamilyName(
...
def kernel32_GetCurrentPackageFullName(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentPackageFullName(
...
def kernel32_GetCurrentPackageId(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentPackageId(
        UINT32* bufferLength,
        BYTE* buffer
    )
    """
...
def kernel32_GetCurrentPackageInfo(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentPackageInfo(
        [PACKAGE_FILTER_FLAGS] flags,
        UINT32* bufferLength,
        BYTE* buffer,
        UINT32* count
    )
...
def kernel32_GetCurrentPackagePath(jitter):
    """
    [ERROR_CODE_LONG] GetCurrentPackagePath(
        UINT32* pathLength,
        PWSTR path
    )
    """
...
def kernel32_GetPackageFamilyName(jitter):
    """
    [ERROR_CODE_LONG] GetPackageFamilyName(
        HANDLE hProcess,
        UINT32* packageFamilyNameLength,
        PWSTR packageFamilyName
    )
...
def kernel32_GetPackageFullName(jitter):
    """
    [ERROR_CODE_LONG] GetPackageFullName(
        HANDLE hProcess,
        UINT32* packageFullNameLength,
        PWSTR packageFullName
    )
...
def kernel32_GetPackageInfo(jitter):
    """
    [ERROR_CODE_LONG] GetPackageInfo(
        PACKAGE_INFO_REFERENCE packageInfoReference,
        [PACKAGE_CONSTANTS] flags,
        UINT32* bufferLength,
        BYTE* buffer,
        UINT32* count
    )
...
def kernel32_GetPackagePath(jitter):
    """
    [ERROR_CODE_LONG] GetPackagePath(
...
def kernel32_GetPackagesByPackageFamily(jitter):
    """
    [ERROR_CODE_LONG] GetPackagesByPackageFamily(
        PWSTR packageFamilyName,
        UINT32* count,
...
def kernel32_OpenPackageInfoByFullName(jitter):
    """
    [ERROR_CODE_LONG] OpenPackageInfoByFullName(
...
def kernel32_PackageFamilyNameFromFullName(jitter):
    """
    [ERROR_CODE_LONG] PackageFamilyNameFromFullName(
        PCWSTR packageFullName,
        UINT32* packageFamilyNameLength,
        PWSTR packageFamilyName
    )
...
def kernel32_PackageFamilyNameFromId(jitter):
    """
    [ERROR_CODE_LONG] PackageFamilyNameFromId(
        const PACKAGE_ID* packageId,
        UINT32* packageFamilyNameLength,
        PWSTR packageFamilyName
    )
...
def kernel32_PackageFullNameFromId(jitter):
    """
    [ERROR_CODE_LONG] PackageFullNameFromId(
        const PACKAGE_ID* packageId,
        UINT32* packageFullNameLength,
        PWSTR packageFullName
    )
...
def kernel32_PackageIdFromFullName(jitter):
    """
    [ERROR_CODE_LONG] PackageIdFromFullName(
        PCWSTR packageFullName,
        [PACKAGE_INFORMATION_FLAGS] flags,
        UINT32* bufferLength,
        BYTE* buffer
    )
    """
...
def kernel32_PackageNameAndPublisherIdFromFamilyName(jitter):
    """
    [ERROR_CODE_LONG] PackageNameAndPublisherIdFromFamilyName(
...
def kernel32_K32GetDeviceDriverBaseName(jitter, get_str, set_str):
    """
    DWORD K32GetDeviceDriverBaseName(
        LPVOID ImageBase,
        LPTSTR lpBaseName,
        DWORD nSize
    )
...
def kernel32_K32GetDeviceDriverFileName(jitter, get_str, set_str):
    """
    DWORD K32GetDeviceDriverFileName(
        LPVOID ImageBase,
        LPTSTR lpFilename,
        DWORD nSize
    )
...
def kernel32_K32GetProcessImageFileName(jitter, get_str, set_str):
    """
    DWORD K32GetProcessImageFileName(
        [ProcessHandle] hProcess,
...
def kernel32_K32InitializeProcessForWsWatch(jitter):
    """
    BOOL K32InitializeProcessForWsWatch(
        [ProcessHandle] hProcess
    )
    """
    ret_ad, args = jitter.func_args_stdcall(["hProcess"])
...
def kernel32_GetProcessPreferredUILanguages(jitter):
    ...
    BOOL GetProcessPreferredUILanguages(
        [MUI_LANGUAGE_FORMAT_FLAGS] dwFlags,
...
def kernel32_GetSystemPreferredUILanguages(jitter):
    ...
    BOOL GetSystemPreferredUILanguages(
        [GetSystemPreferredUILanguages_FLAGS] dwFlags,
        PULONG pulNumLanguages,
        PZZWSTR pwszLanguagesBuffer,
        PULONG pcchLanguagesBuffer
    )
...
def kernel32_GetThreadPreferredUILanguages(jitter):
    ...
    BOOL GetThreadPreferredUILanguages(
        [GetThreadPreferredUILanguages_FLAGS] dwFlags,
        PULONG pulNumLanguages,
        PZZWSTR pwszLanguagesBuffer,
        PULONG pcchLanguagesBuffer
    )
...
def kernel32_GetUILanguageInfo(jitter):
    ...
    BOOL GetUILanguageInfo(
        ...
        [MUI_LANGUAGE_ATTRIBUTES*] pdwAttributes
    )
    """
...
def kernel32_SetThreadPreferredUILanguages(jitter):
    ...
    BOOL SetThreadPreferredUILanguages(
        [SetThreadPreferredUILanguages_FLAGS] dwFlags,
        PCZZWSTR pwszLanguagesBuffer,
        PULONG pulNumLanguages
    )
    """
...
def kernel32_GetEnabledXStateFeatures(jitter):
    """
    [XSTATE_FEATURE_64] GetEnabledXStateFeatures()
    """
    ret_ad, args = jitter.func_args_stdcall([])
    raise RuntimeError('API not implemented')
    jitter.func_ret_stdcall(ret_ad, ret_value)
